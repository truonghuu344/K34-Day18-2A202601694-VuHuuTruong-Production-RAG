    ...
    for parent in parents:
        ...
        for para in parent_paras:
            if len(para) > child_size:
                ...
                c_sub = ""
                for s in sentences:
                    if len(c_sub) + len(s) > child_size and c_sub:
                        children.append(Chunk(text=c_sub.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
                        c_sub = ""
                    c_sub += s + " "
                if c_sub.strip():
                    children.append(Chunk(text=c_sub.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
            else:
                if len(current_child_text) + len(para) > child_size and current_child_text:
                    children.append(Chunk(text=current_child_text.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
                    current_child_text = ""
                current_child_text += para + "\n\n"
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    for item in sections:
        if not item:
            continue
        if re.match(r'^#{1,3}\s+.+', item.strip()):
            combined = (current_header + "\n\n" + current_content).strip() if current_header else current_content.strip()
            if combined:
                chunks.append(Chunk(text=combined, metadata={**metadata, "section": current_header or "header", "strategy": "structure"}))
